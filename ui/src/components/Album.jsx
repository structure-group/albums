...
export default function Album() {
    ...
    const album = useQuery({ queryKey: ["album", ship, albumId], queryFn: () => albumQuery(albumId, ship) });
                ...
                {album?.data?.album?.images?.map((image) => {
                    return (
                        <div className="w-32 h-32" key={image[1]?.src}>
                            <img src={image[1]?.src} alt="" className="w-32 h-32 object-contain" />
                        </div>
                    )
                })}
                {ship === `~${window.ship}` && <div className="flex flex-col items-center justify-center border-[#999999] border rounded-lg w-32 h-32 font-semibold">
                    + Add Photo
                </div>}
            </div>
        </div>
