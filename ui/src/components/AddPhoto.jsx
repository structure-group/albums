...
export default function AddPhoto({ setAddPhoto }) {
    ...
    const Cell = ({ columnIndex, rowIndex, style }) => {
        ...
        return (
            <div className={cn("p-1 cursor-pointer border", {
                "pointer-events-none opacity-25": album?.images?.some((image) => image[1]?.src === url),
    ...
    const addPhotos = () => {
        const promises = selectedFiles.map((url) => {
            console.log({ "add": { "album-id": { "name": albumId, "owner": ship }, "img-id": uuidv4(), "src": url, "caption": { "who": window.ship, "when": Math.floor(Date.now() / 1000), "what": "" } } })
            return api.poke({
                app: "albums",
                mark: "albums-action",
                json: { "add": { "album-id": { "name": albumId, "owner": ship }, "img-id": uuidv4(), "src": url, "caption": { "who": `~${window.ship}`, "when": String(Math.floor(Date.now() / 1000)), "what": "" } } },
            });
        }
        );
            ...
            <Foco onClickOutside={() => setAddPhoto(false)}>
                <div className="bg-white rounded-xl w-[600px] h-[400px] flex flex-col items-center justify-center z-10 p-4">
                    ...
                    <div className="flex space-x-2 pt-2 w-full">
                        ...
                        >Cancel</button>
                        <button className="text-sm font-semibold bg-black text-white w-full rounded-md py-1"
                            onClick={() => addPhotos()}
                        >Add</button>
                    </div>
                </div>
