...
function parseFolderIntoTree(folders, previous) {
...
export function searchTree(tree, stop) {
  if (stop(tree)) {
    return tree;
  }
...
export function traverseTree(partialPath, tree, stop) {
  const segments =
    ...
    (set, get) => ({
      ...
      setCurrentFile: (key, folder) => {
        const file = get().files.find(
          (file) =>
            `${file.folder === "/" ? "" : file.folder}/${file.filename}` ===
            key,
        );
      ...
      removeFolder: async (folder, s3) => {
        ...
        const parentPath = normalizeRoot(folder.path)
          .split("/")
          .slice(0, -1)
          .join("/");
